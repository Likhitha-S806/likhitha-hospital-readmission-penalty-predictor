...
if predict_button:
    ...
    if "Single Model" in prediction_mode:
        with st.spinner(f"Training {selected_model_name} and analyzing..."):
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                delta={'reference': 50},
                title={'text': f"Penalty Risk - {selected_model_name}"},
    ...
    with col1:
        st.metric(
            "📊 Consensus",
            "PENALIZED" if consensus_prediction == 1 else "NOT PENALIZED"
        )
    with col2:
        st.metric(
            "🎲 Average Probability",
    ...
    with col3:
        st.metric(
            "✅ Models Agree",
            f"{sum(all_predictions)}/{len(all_predictions)}"
        )
    with col4:
        st.metric(
            "📈 Excess Ratio",
    ...
    st.caption(
        "✅ = Not Penalized, ⚠️ = Penalized. Higher probability means higher penalty risk."
    )

    results_df = pd.DataFrame({
        "Model": list(model_results.keys()),
        "Prediction": [
            "PENALIZED" if v["prediction"] == 1 else "NOT PENALIZED"
